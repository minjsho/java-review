...
//		animal.walk();
		//자식클래스에서 추가한 멤버는 부모타입의 참조변수로 접근할 수 없다
		//이 경우에는 다운캐스팅을 해야한다
//		((Dog)animal).walk();
		...
		printText(dog);
		printText(cat);
		printText(new Dog("멍뭉이")); //객체를 생성하지 않고 가능
...
//	}
	...
	//2.같은 부모 클래스가 존재한다면 매개변수를 부모클래스 타입으로 받는다(업캐스팅)
	//하나의 메소드에서 매개변수를 부모타입으로 만들면 여러 자식 타입을 받아서 사용이 가능하다(매개변수(참조변수)의 다형성)
	static void printText(Animal animal) {
		System.out.println(animal.name + "이가 운다");
		animal.crying();
	}
	
	//3.만약 부모 클래스에 존재하지 않는 멤버에(자식클래스에서 생성한 멤버) 접근하기 위해서는
	//다운캐스팅 해줘야 함
	static void tryWalk(Animal animal) {
		//매개변수로 받아온 객체의 주소가 cat 타입인지ㅡ dog 타입인지 확인해야 한다
